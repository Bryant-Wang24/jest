    ...
    >
      ...
      <el-table-column label="操作" :resizable="false" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-button v-if="hasRolePermission('AccountManage_orderRecall')" type="text" @click="recall(scope.row)">撤回</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    async recall(row) {
      const { projectId, customerId, id } = row
      const params = {
        projectId,
        customerId
      }
      ...
      if (invoices.length > 0) {
        html = `<div class="confirm">
          ...
          <span class="tip">撤回后金额将从订单重新返回台账中</span>
        </div>`
      } else {
        html = `<div class="confirm">
